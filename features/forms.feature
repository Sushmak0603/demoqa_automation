Feature: Practice Forms Testing
  ...
  Background:
    ...
    Scenario: Verify if the Form does not get submitted when no field is entered (TC005)
    ...
    Scenario: Verify the Mobile Number field accept only numbers, of 10 digits (TC006)
...
#
  Scenario: Verify the Email field accept only valid email-ids (TC007
        When the user enters "Sushma" in "First Name" field
        When the user enters "K" in "Last Name" field
        When the user selects "Female" gender radio button
        When the user enters "1234567890" in "Mobile Number" field
